...
def feistel(R, subkey):
    ...
    xored = "".join(
        str(int(a) ^ int(b))
        for a, b in zip(expanded, subkey)
    )
...
def des_encrypt(plain_hex, key_hex):
    ...
    plain_bits = hex_to_bin(plain_hex)
    ...
    for i in range(16):
        ...
        R = "".join(
            str(int(l) ^ int(f))
            for l, f in zip(L, f_out)
        )

        L = temp_R

        # Print only the intermediate 64-bit value
        print(f"Round {i + 1:2d}: {bin_to_hex(L + R)}")
...
plaintext_input = "123456ABCD132536"
key_input = "AABB09182736CCDD"
